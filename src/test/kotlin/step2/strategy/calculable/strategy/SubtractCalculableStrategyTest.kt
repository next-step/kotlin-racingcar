...
class SubtractCalculableStrategyTest {

    @DisplayName("SubtractCalculableStrategy 인스턴스 생성 여부 테스트")
    @Test
    fun constructor_test() {
        // given and when
        val calculable = SubtractCalculableStrategy()

        // then
        assertAll(
            { assertThat(calculable).isNotNull },
            { assertThat(calculable).isExactlyInstanceOf(SubtractCalculableStrategy::class.java) },
        )
    }
