...
class StringCalculatorTest {
    ...
    fun `Parsing null or blank`() {
        assertThatThrownBy {
            StringCalculator.parsing(stringExpression = null, result = {})
        }.isInstanceOf(IllegalArgumentException::class.java)
            .hasMessageContaining("Null or blank string expression")

        assertThatThrownBy {
            StringCalculator.parsing(stringExpression = "   ", result = {})
        }.isInstanceOf(IllegalArgumentException::class.java)
            .hasMessageContaining("Null or blank string expression")
    }
    ...
    fun `Parsing include regular spacing`() {
        StringCalculator.parsing(stringExpression = " 23 + 3 * 4 / 2 ", result = {
