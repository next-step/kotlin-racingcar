...
class InputParserTest {
    ...
    fun parseTest() {
        assertThat(parser.parse("4 + 5 - 2").getOrThrow())
            .isEqualTo(Pair(listOf(4.0, 5.0, 2.0), listOf("+", "-")))
        assertThat(parser.parse("3 + 4 / 2").getOrThrow())
            .isEqualTo(Pair(listOf(3.0, 4.0, 2.0), listOf("+", "/")))
    ...
    @Test
    fun validateTest() {
        assertThat(parser.validate(Pair(listOf(4.0, 5.0, 2.0), listOf("+", "-"))).getOrThrow())
            .isEqualTo(Pair(listOf(4.0, 5.0, 2.0), listOf("+", "-")))
        assertThatThrownBy { parser.validate(Pair(listOf(0.0, -4.0, 2.0), listOf("+", "/"))).getOrThrow() }
            .isInstanceOf(IllegalArgumentException::class.java)
            .hasMessageContaining("모든 입력 숫자는 0보다 커야 합니다")
        assertThatThrownBy { parser.validate(Pair(listOf(3.0, 4.0, 2.0), listOf("?", "/"))).getOrThrow() }
            .isInstanceOf(IllegalArgumentException::class.java)
