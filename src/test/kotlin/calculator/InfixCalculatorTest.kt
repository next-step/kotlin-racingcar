...
class InfixCalculatorTest {
    ...
    fun `수식에 숫자와 사칙 연산이 아닌 문자가 있을 경우 IllegalArgumentException`(input: String) {
        Assertions.assertThatThrownBy { InfixCalculator(input).parse() }
    ...
    fun `연산자가 숫자 수 보다 많거나 같은 경우`() {
        Assertions.assertThatThrownBy { InfixCalculator("10 / 20 * 30 + 10 - ").parse() }
            .isInstanceOf(IllegalArgumentException::class.java)
    ...
    @Test
    fun `null 인 경우`() {
        Assertions.assertThatThrownBy { InfixCalculator(null).parse() }
            .isInstanceOf(IllegalArgumentException::class.java)
    ...
    @Test
    fun `공백 문자일 경우`() {
        Assertions.assertThatThrownBy { InfixCalculator("").parse() }
            .isInstanceOf(IllegalArgumentException::class.java)
    ...
    @Test
    fun `정상적인 계산식 일 경우`() {
        val infixOperation = InfixCalculator("2 + 3 * 4 * 2 / 2 - 5 ").parse()
        val result = infixOperation.operate()
