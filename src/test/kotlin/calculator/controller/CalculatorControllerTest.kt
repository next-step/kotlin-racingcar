...
class CalculatorControllerTest {
    ...
    fun `연산된 결과를 반환한다`(number1: String, operator: String, number2: String, expected: Int) {
        assertEquals(expected, CalculatorController(listOf(number1, operator, number2)).getResult())
    }

    @Test
    fun `피연산자가 1개인 경우 그대로 반환한다`() {
        assertEquals(5, CalculatorController(listOf("5")).getResult())
    }

    @Test
    fun `첫 피연산자가 숫자가 아닌경우 예외를 던진다`() {
        assertThatThrownBy { CalculatorController(listOf("?", "+", "2")).getResult() }
    ...
    fun `숫자와 연산자 사이가 공백으로 이뤄지지 않은 경우 예외를 던진다`() {
        assertThatThrownBy { CalculatorController(listOf("1", "-2")).getResult() }
            .isInstanceOf(IllegalArgumentException::class.java)
    ...
    fun `피연산자를 입력하지 않은 경우 예외를 던진다`() {
        assertThatThrownBy { CalculatorController(listOf("1", "*")).getResult() }
            .isInstanceOf(IllegalArgumentException::class.java)
