package study

import org.assertj.core.api.Assertions
import org.junit.jupiter.api.Test

class DirectionTest {

    @Test
    fun plus_minus_multiplication_division() {
        val numList = listOf(2, 1)
        val direction = Direction()

        val plus = direction.plus(numList)
        val minus = direction.minus(numList[0], numList[1])
        val multiplication = direction.multiplication(numList[0], numList[1])
        val division = direction.division(numList[0], numList[1])

        Assertions.assertThat(plus).isEqualTo(3)
        Assertions.assertThat(minus).isEqualTo(1)
        Assertions.assertThat(multiplication).isEqualTo(2)
        Assertions.assertThat(division).isEqualTo(2)
    }

    @Test
    fun find_symbol() {
        val direction = Direction()

        val plusSymbol = direction.isPlus("+")
        val minusSymbol = direction.isMinus("-")
        val multiplicationSymbol = direction.isMultiplication("*")
        val divisionSymbol = direction.isDivision("/")

        Assertions.assertThat(plusSymbol).isTrue()
        Assertions.assertThat(minusSymbol).isTrue()
        Assertions.assertThat(multiplicationSymbol).isTrue()
        Assertions.assertThat(divisionSymbol).isTrue()
    }

    @Test
    fun division_has_zero() {
        val a = 2
        val b = 0
        val direction = Direction()

        try {
            direction.division(a, b)
        } catch (e: ArithmeticException) {
            Assertions.assertThat(e.message).isEqualTo("0의값으로 나눌순 없습니다")
        }
    }

    @Test
    fun not_symbol() {
        val symbol = "a"
        val numList = mutableListOf<Int>(1, 2)
        val direction = Direction()

        try {
            direction.calculate(symbol, numList)
        } catch (e: IllegalArgumentException) {
            Assertions.assertThat(e.message).isEqualTo("사칙연산 기호가 아닙니다")
        }
    }
}
