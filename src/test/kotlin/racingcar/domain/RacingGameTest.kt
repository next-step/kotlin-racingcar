...
class RacingGameTest {
    ...
    fun `시도 횟수가 1보다 작으면 예외가 발생한다`() {
        val exception = shouldThrowExactly<IllegalArgumentException> {
            RacingGame(attemptCount = 0, cars = listOf(Car(name = "pobi") { true }))
        }
    ...
    @Test
    fun `게임을 생성할 수 있다`() {
        val actual = RacingGame(attemptCount = 1, cars = listOf(Car(name = "pobi") { true }))
    ...
    fun `게임을 실행하면 시도 횟수를 1만큼 차단하고 자동차들을 움직인다`() {
        val racingGame = RacingGame(attemptCount = 1, cars = listOf(Car(name = "pobi") { true }))
        racingGame.run()
    ...
    fun `게임이 실행 가능하면 true를 반환한다`() {
        val actual = RacingGame(attemptCount = 1, cars = listOf(Car(name = "pobi") { true })).isRunnable()
