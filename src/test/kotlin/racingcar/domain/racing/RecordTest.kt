...
internal class RecordTest {
    ...
    fun `기록은 생성될 때의 차의 이름과 위치를 그대로 반환한다`(name: String, location: Int) {
        ...
        assertAll(
            { Assertions.assertThat(record.name).isEqualTo(CarName(name)) },
            { Assertions.assertThat(record.location).isEqualTo(CarLocation(location)) }
        )
    }
    ...
    fun `다른 기록의 위치와 같으면 true를 반환한다`(location: Int) {
        ...
        Assertions.assertThat(one.isSameLocationAs(other)).isTrue()
    }
    ...
    fun `Record는 객체가 달라도 값이 같으면 동일하다`(name: String, location: Int) {
        Assertions.assertThat(Record(name, location)).isEqualTo(Record(CarName(name), CarLocation(location)))
    }
    ...
    fun `다른 기록의 위치와 다르면 false를 반환한다`(oneLocation: Int, otherLocation: Int) {
        ...
        Assertions.assertThat(one.isSameLocationAs(other)).isFalse()
    }
}
