...
import racingcar.domain.PreparedMovementCommandGenerator
    ...
    describe("경주 게임 관리자는") {
        context("주어진 입력이 유효하다면") {
            ...
            it("게임을 진행하고 게임 결과를 반환한다(case: 우승자 1명)") {
                val movementsGenerator = PreparedMovementCommandGenerator(movementsWithOneWinner)
            ...
            it("게임을 진행하고 게임 결과를 반환한다(case: 우승자 여러명)") {
                val movementsGenerator = PreparedMovementCommandGenerator(movementsWithThreeWinners)
                val gameResult = RacingGameManager.proceed(gameInput, movementsGenerator)
                gameResult.also {
                    it.raceResults.size shouldBe numberOfRaces
                    it.raceResults.forEach { raceResult ->
