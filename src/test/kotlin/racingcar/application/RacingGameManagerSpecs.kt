    ...
    describe("경주 게임 관리자는") {
        context("주어진 자동차의 이름 수가 비어있다면") {
            ...
            it("예외를 발생시킨다") {
                shouldThrowExactly<IllegalArgumentException> {
                    RacingGameManager.play(gameInput, RandomDriver)
                }
            }
        }
        ...
        context("주어진 입력이 유효하다면") {
            ...
            it("게임을 진행하고 게임 결과를 반환한다(case: 우승자 1명)") {
                val driver = PreparedDriver(movementsWithOneWinner)
                val gameResult = RacingGameManager.play(gameInput, driver)
                gameResult.also {
            ...
            it("게임을 진행하고 게임 결과를 반환한다(case: 우승자 여러명)") {
                ...
                val gameResult = RacingGameManager.play(gameInput, movementsGenerator)
                gameResult.also {
