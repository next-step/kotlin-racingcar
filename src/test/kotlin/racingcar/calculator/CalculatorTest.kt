...
class CalculatorTest {
    ...
    fun `계산할 연산식 입력받고 숫자만 추출`() {
        val cal = Calculator(UserInputString("3 + 5 - 1"))
    ...
    fun `계산기 초기화 후 계산수행 1`() {
        val userInput = "3 + 5 - 1"
        val userInputString = UserInputString(userInput)
        val cal = Calculator(userInputString)
        assertThat(cal.calculate()).isEqualTo(7.0)
    }

    @Test
    fun `계산기 초기화 후 계산수행 2`() {
        val cal = Calculator(UserInputString("2 * 5 / 2"))
