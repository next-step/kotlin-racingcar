...
class CarGameTest : StringSpec({
    "게임 진행을 위해 입력받은 이름목록의 길이나 회차의 값이 양수가 아니라면 게임이 생성되지 않고 예외를 반환한다." {
        forAll(
            row(listOf(), 3),
            row(listOf("test"), 0),
            row(listOf("test"), -1),
    ...
    "게임 생성 시 참여하는 차량 대수와 진행할 회차가 입력받은 값으로 설정된다. " {
        val game = CarGame(listOf("test1", "test2", "test3"), 5, FixedNumberGenerator(4))
    ...
    "게임이 종료된 이후는 경주를 실행할 수 없습니다." {
        val game = CarGame(listOf("test1", "test2", "test3"), 1, FixedNumberGenerator(4))
        game.playSinglePhase()
    ...
    "회차를 진행할 때 4 미만의 숫자가 전달된 차량은 전진하지 못한다." {
        val game = CarGame(listOf("test1", "test2", "test3"), 5, FixedNumberGenerator(3))
        game.playSinglePhase()
