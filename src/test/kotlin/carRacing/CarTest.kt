...
class CarTest {
    ...
    fun `자동차가 전진할 경우 position 값이 증가한다`() {
        val car: Car = Car(movementController = TestTrueMovementController(), position = 1)
    ...
    fun `자동차가 전진하지 못할 경우 position 값이 증가하지 않는다`() {
        val car: Car = Car(movementController = TestFalseMovementController(), position = 1)
    ...
    fun `position 값 없이 생성된 자동차의 position은 1이다`() {
        val car: Car = Car(movementController = TestTrueMovementController())
    ...
    fun `position 값이 주어진 자동차의 position은 주어진 값이다`() {
        val car: Car = Car(movementController = TestTrueMovementController(), position = 3)
