...
object InputView {
    fun inputRacingCarInfo(): Pair<String, Int> {
        val racingCarNames: String = inputRacingCarNames()

        val repeatTime: Int = inputRacingRepeatTime()
        return racingCarNames to repeatTime
    }

    private fun inputRacingRepeatTime(): Int {
        println("시도할 횟수는 몇 회인가요?")
        val repeatTime: String? = readLine()

        requireNotNull(repeatTime) { "시도 횟수를 입력해야 합니다" }
        require(repeatTime.isInt()) { "숫자만 입력 가능합니다" }
        return repeatTime.toInt()
    }

    private fun inputRacingCarNames(): String {
