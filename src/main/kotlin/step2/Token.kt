...
data class Token private constructor(
    ...
    fun getOperatorSign(): OperatorSign {
        check(this.isOperator()) { throw IllegalStateException("연산자 유형인 경우에만 연산 기호를 구할 수 있습니다.") }
        return OperatorSign.getBySign(sign = this.value)
    }

    fun getOperandNumber(): Int {
        check(this.isOperand()) { throw IllegalStateException("피연산자 유형인 경우에만 피연산자 숫자를 구할 수 있습니다.") }
