...
## 추가요구사항 이슈
* 각 차량의 이동거리가 고유하기 위해서는 반드시 라운드수가 "차량수-1"인 값과 일치해야 경우의 수가 가능하다.(DM 드렸을때는 일치해야 한다고 말씀드렸는데요. 생각해보니 -1이 되어도 충족되네요.)
  * 차량 증가에 따른 각기 다른 이동거리 및 우승자가 당첨되는 시뮬레이션을 시트로 표현해보면 아래와 같음.
![img.png](add_exam_step5.png)
  * 입력시 차량의 수를 먼저 체크하고 이에 따라 라운드를 입력받을때 최소값을 가이드 해야 한다.-> 라운드 도메인 수정 필요.
  * 이동거리에 대한 경우의 수를 만들 방법을 찾아야 한다.
    * 요건을 기준으로 아래와 같은 상세조건을 도출하였다.
    * ![img.png](solo_winner_pattern.png)
      *   라운드 전체 기준 차량별 이동여부의 가지수는 차량 수 * 라운드수 / 2
      *   전체 경의 수를 기준으로 이동 가능/불가의 비율은 5:5로 구성된다.
...
## 요구사항을 책임단위로 세분화
* controller 패키지 생성 및 구현.
  * 컨트롤러에서 계약모델 view를 반환하는 구조로 이해하고 racing에서 경기 내용과 경기 결과를 각각 나눠 받아서 하나의 view로 완성해야 한다.
    * 해당 작업을 진행하면 자연스럽게 playRacing 에서 view 영역이 분리되면서 추가요청사항을 다룰 수 있을 것 같다.
* racing 클래스 개선.
  * playRacing 안에 다루고 있는 view 영역의 분리.
  * 기존 경기진행과 결과를 다루는 하나의 함수를 분리.
    * 경기 내용을 다루는 함수 생성.
      * 매라운드별 결과를 콜랙션으로 담아야 한다.(list<Cars> 같은 구조가 될 것으로 예상.)
      * 이 라운드별 레이싱상황을 담는 콜랙션은 별다른 비지니스가 없어서 일급 콜랙션화는 일단 보류한다.
    * 경기 결과를 다루는 함수 생성.
* MoveStrategy 추가
  * 우승자를 1명 지정하고 이에 따른 차등 판별이 되도록 구성해본다.
